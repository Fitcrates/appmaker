...
export const useStudioFilter = () => {
  const { selectedStudios, setSelectedStudios } = useAnimeStore();
  ...
  const handleStudioSelect = useCallback((studio: Studio) => {
    const newSelectedStudios = [...selectedStudios];
    const existingIndex = newSelectedStudios.findIndex(s => s.mal_id === studio.mal_id);
    
    if (existingIndex !== -1) {
      newSelectedStudios.splice(existingIndex, 1);
    } else {
      newSelectedStudios.push(studio);
    }
    
    setSelectedStudios(newSelectedStudios);
  }, [selectedStudios, setSelectedStudios]);
