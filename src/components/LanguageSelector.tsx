...
import { useGoogleTranslate } from '../hooks/useGoogleTranslate'; // Import the useGoogleTranslate hook
...
export function LanguageSelector() {
  ...
  const { isReady, changeLanguage: changeLanguageHook } = useGoogleTranslate(); // Use the hook
  ...
  const changeLanguage = (lang: Language) => {
    try {
      ...
      // If Original language is selected, reset translation
      if (lang.code === '') {
        resetTranslation();
        return;
      }
      
      // Use the hook's changeLanguage function for RTL support
      if (isReady && lang.code) {
        changeLanguageHook(lang.code);
        return;
      }
      
      // Fallback to the original implementation if the hook is not ready
  ...
  // Function to reset translation back to original
  const resetTranslation = () => {
    ...
    // Reload the page to ensure translation is completely reset
    window.location.reload();
  };
